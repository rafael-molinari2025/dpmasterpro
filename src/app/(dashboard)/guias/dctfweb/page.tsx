...
export default async function GuiasDCTFWebPage({
  searchParams,
}: {
      ...
      <div className="flex-1 p-3 sm:p-6 space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex gap-3">
          ...
          <div className="text-sm text-blue-800">
            <p className="font-medium">DCTFWeb — Declaração mensal obrigatória</p>
            <p className="mt-0.5 text-blue-700">
              <strong>Vencimento:</strong> Dia 15 do mês subsequente à competência.
              A DCTFWeb é gerada automaticamente a partir dos dados do eSocial (S-1200, S-1210) e da EFD-Reinf.
              Substitui a GFIP para declaração de contribuições previdenciárias.
            </p>
          </div>
        </div>
        ...
        {guias.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 text-center py-16">
            ...
            <p className="text-gray-500 font-medium">Nenhuma DCTFWeb gerada</p>
            <p className="text-sm text-gray-400 mt-1">As declarações são geradas ao processar a folha de pagamento.</p>
          </div>
        ) : (
          <>
            {/* Cards (mobile) */}
            <div className="sm:hidden space-y-3">
              ...
              <p className="text-xs text-gray-400 text-center pt-1">{guias.length} DCTFWeb gerada{guias.length !== 1 ? "s" : ""}</p>
            </div>
              ...
              <table className="w-full min-w-[520px]">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="text-right px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Valor Total</th>
