...
export default function CargosAcoes({ empresas }: { empresas: Empresa[] }) {
  const router = useRouter();
  const [aberto, setAberto] = useState(false);
  const [salvando, setSalvando] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [form, setForm] = useState({ empresaId: empresas[0]?.id ?? "", codigo: "", descricao: "", cbo: "", salarioBase: "" });
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/cargos", {
        ...
        body: JSON.stringify({ ...form, salarioBase: parseFloat(form.salarioBase) || 0 }),
      });
      const data = await res.json();
      if (!res.ok) { setErro(data.error ?? "Erro ao salvar."); return; }
      setAberto(false);
      setForm({ empresaId: empresas[0]?.id ?? "", codigo: "", descricao: "", cbo: "", salarioBase: "" });
      router.refresh();
    } catch {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-semibold text-gray-900">Novo Cargo</h2>
            ...
            <form onSubmit={salvar} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" required maxLength={20} placeholder="Ex: ADM, TEC01" className={inp} {...f("codigo")} />
                </div>
                <div>
                  <label className={lb}>CBO</label>
                  <input type="text" maxLength={10} placeholder="Ex: 2124-05" className={inp} {...f("cbo")} />
                </div>
              </div>
              <div>
                <label className={lb}>Descrição *</label>
                <input type="text" required maxLength={100} placeholder="Ex: Analista de Sistemas" className={inp} {...f("descricao")} />
              </div>
              <div>
                <label className={lb}>Salário Base</label>
                <input type="number" min="0" step="0.01" placeholder="0,00" className={inp} {...f("salarioBase")} />
              </div>
